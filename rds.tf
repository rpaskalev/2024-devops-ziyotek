resource "aws_db_instance" "default" {
  allocated_storage      = 10
  db_name                = var.rds_name
  engine                 = "postgres"
  engine_version         = "10.15"
  instance_class         = "db.t3.micro"
  username               = var.user_name
  password               = aws_ssm_parameter.pass_gen.value #random_password.password.result # "foobarbaz"
  parameter_group_name   = "default.mysql10"
  skip_final_snapshot    = true
  db_subnet_group_name   = aws_db_subnet_group.ziyo_subg.id
  vpc_security_group_ids = [aws_security_group.ziyo_security_group.id]
  storage_type           = "gp2"
}

resource "aws_db_subnet_group" "ziyo_subg" {
  name       = "ziyo_syb_group"
  subnet_ids = [aws_subnet.ziyo_subnet_public.id, aws_subnet.ziyo_subnet_private.id]

}

resource "random_password" "password" {
  length  = 7
  special = true
  #override_special = "!#$%&*()-_=+[]{}<>:?"
}

resource "aws_ssm_parameter" "pass_gen" {
  name  = "ziyo-rds-pass"
  type  = "String"
  value = random_password.password.result
}